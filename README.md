...
- 리더의 일정 생성 (제목, 날짜, 장소, 정원 설정)
- 밴드 멤버 참여 신청 / 취소
- 리더의 참여자 목록 조회
- **동시성 처리** — 여러 멤버가 마지막 자리에 동시에 신청해도 정원 초과 불가 (비관적 락)
...
Band (1) ──── (n) Rehearsal             [합주 일정]
Rehearsal (1) ─ (n) RehearsalAttendance [참여 신청]
```
...
| RehearsalAttendance | (rehearsalId, userId) |
...
| POST | `/` | 리더만 | 합주 일정 생성 |
| GET | `/` | - | 밴드 합주 일정 목록 |
| GET | `/{rehearsalId}` | - | 합주 일정 단건 조회 |
| POST | `/{rehearsalId}/join` | 멤버만 | 참여 신청 |
| DELETE | `/{rehearsalId}/join` | 멤버만 | 참여 취소 |
| GET | `/{rehearsalId}/attendances` | 리더만 | 참여자 목록 조회 |
...
// Response
{ "id": 1, "bandId": 1, "title": "정기 합주", "maxCapacity": 5, "currentCount": 0, ... }
```

**POST /api/bands/1/rehearsals/1/join**
```json
// Response
{ "id": 1, "rehearsalId": 1, "userId": 2, "createdAt": "..." }
```
</details>
...
### 3. 동시성 처리 — 합주 정원 초과 방지

합주 일정 참여 신청 시 다수 유저가 동시에 마지막 자리에 신청하는 경우를 **비관적 락**으로 처리합니다.

```java
// RehearsalRepository — SELECT FOR UPDATE 발행
...
// 1) row lock 획득 — 다른 트랜잭션은 이 시점부터 대기
Rehearsal rehearsal = rehearsalRepository.findByIdWithLock(rehearsalId)...;

// 2) 정원 확인 (락 보호 아래 체크하므로 동시 요청과 무관하게 안전)
if (rehearsal.getCurrentCount() >= rehearsal.getMaxCapacity()) {
    throw new RuntimeException("정원이 초과되었습니다.");
}

// 3) 신청 저장 + currentCount 증가 → 트랜잭션 커밋 시 lock 해제
attendanceRepository.save(attendance);
rehearsal.setCurrentCount(rehearsal.getCurrentCount() + 1);
```

**왜 비관적 락인가?**
- 합주 신청은 마감 직전 요청이 몰리는 패턴 → 충돌 빈도가 높음
- 낙관적 락은 충돌 시 예외를 던져 재시도를 강제하지만, 여기서는 실패를 바로 알려주는 게 UX에 적합
- `(rehearsal_id, user_id)` unique 제약으로 중복 신청은 DB 레벨에서도 방어

**동시성 검증 테스트** (`RehearsalServiceConcurrencyTest`)

```java
// 정원 5명, 동시 신청 10명 → 정확히 5명만 성공하는지 검증
CountDownLatch startLatch = new CountDownLatch(1);
// 10개 스레드가 startLatch.await()으로 대기 후 동시에 출발
startLatch.countDown();
doneLatch.await();

assertThat(successCount.get()).isEqualTo(5);
assertThat(rehearsal.getCurrentCount()).isEqualTo(5);
```
...
│   ├── service/RehearsalService.java
│   ├── entity/ (Rehearsal, RehearsalAttendance)
│   ├── dto/   (CreateRehearsalRequest, RehearsalResponse, AttendanceResponse)
│   └── repository/ (RehearsalRepository, RehearsalAttendanceRepository)
